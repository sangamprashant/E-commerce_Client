...
router.post("/api/make/order", requireLogin, async (req, res) => {
    try {
      const { name,email,city,postalCode,street,country,CartProducts,phone,APhone,total,paid,paidStatus } = req.body;
      ...
      const order = new Order({
        name, email, city, postalCode, street, country,line_items,phone,APhone,total,paid,status:"confirm"
      })
  ...
  // router.post("/api/make/order/byCash", requireLogin, async (req, res) => {
  //   try {
  //     const { name,email,city,postalCode,street,country,CartProducts,phone,APhone,total,paid } = req.body;
  //     const user = await User.findById({ _id: req.user._id });
  //     if (!user) {
  //       return res.status(404).json({ message: "User not found" });
  //     }
